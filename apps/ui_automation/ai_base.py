            ...
            if len(history) > last_processed_step:
                for i in range(last_processed_step, len(history)):
                    ...
                    try:
                        ...
                        # 检查所有子任务是否都已处于终端状态，是则自动停止
                        if planned_tasks and all(
                            task.get('status', 'pending') in ('completed', 'failed', 'skipped')
                            for task in planned_tasks
                        ):
                            logger.info("All planned tasks completed, stopping agent loop")
                            agent_instance._task_was_done = True
                            _task_was_done = True
                            raise KeyboardInterrupt("All tasks completed")
